...
def bootstrap(y0,z0,conf=THE.bs.conf,b=THE.bs.b):
  ...
  for i in range(b):
    if testStatistic(Sum([one(yhat) for _ in yhat]),
      ...
      bigger += 1
  print(dict(ymu=y.mu,  zmu=z.mu, tobs=baseline, more=bigger/b))
...
if __name__ == "__main__":
  a= [1,3,5,6,8,8,9,9,14,14,17,22,29,29,31,40,
      42,44,46,46,46,47,60,65,67,69,79,84,87,88,89,91]
  b= [1.01,3.03,5.05,6.06,8.08,8.08,9.09,9.09,14.14,14.14,
      17.17,22.22,29.29,29.29,31.31,40.4,42.42,44.44,46.46,
      46.46,46.46,47.47,60.6,65.65,67.67,69.69,
      79.79,84.84,87.87,88.88,89.89,91.91]
  print(bootstrap(a,b))
